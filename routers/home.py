...
from schemas import HomeFeedRequest
from core.config import MACRO_CLUSTERS_CACHE, TIME_RULES_CACHE, WEATHER_CACHE_STORE
from services.recommender import calculate_user_vector, build_cluster_fts_query

router = APIRouter()

@router.post("/api/home/{uid}")
def get_dynamic_home_feed(uid: str, req: HomeFeedRequest, conn: sqlite3.Connection = Depends(get_db_connection)):
    ...
    if activities:
        try:
            ...
            user_vector = calculate_user_vector(conn, activities, calc_decay, current_hour=datetime.now().hour)
